...
class PeerReviewBudgetTests(unittest.TestCase):
    def test_peer_review_degrades_and_caps_second_round(self):
        ...
        self.assertTrue(
            any("Peer review degraded to deterministic_only" in warning for warning in pipeline.last_execution_warnings)
        )
        self.assertTrue(
            any("limited second-round review to 1 highest-risk claims" in warning for warning in pipeline.last_execution_warnings)
        )
        self.assertGreaterEqual(reviewed.cluster_stats["second_round_claim_count"], 1)
