...
class DebateEvidenceToolInput(ToolArgsModel):
    source_id: StrictStr = Field(description="Verifiable source_id or the literal 'llm'.")
    quote: Optional[StrictStr] = Field(default=None, description="Optional supporting quote.")


class DebateProposalPayloadToolInput(ToolArgsModel):
    reaction_type: StrictStr = Field(description="Target reaction type.")
    electrode_composition: StrictStr = Field(description="Electrode composition string.")
    catalyst_metal_elements: List[StrictStr] = Field(
        default_factory=list,
        description="Canonical catalyst metal elements.",
    )
    products: StrictStr = Field(description="Main products or 'N/A'.")
    performance_metrics: StrictStr = Field(description="Single-point performance metric summary.")
    confidence: Literal["low", "medium-low", "medium", "medium-high", "high"] = Field(
        description="Confidence label for the proposal.",
    )
    evidence: List[DebateEvidenceToolInput] = Field(
        default_factory=list,
        description="Evidence citations supporting the proposal.",
    )
    rationale: StrictStr = Field(description="Reasoning and caveats for the proposal.")


class DebateProposalConcludeToolInput(ToolArgsModel):
    conclusion: DebateProposalPayloadToolInput = Field(description="Strict JSON proposal payload.")


class DebateReviewItemToolInput(ToolArgsModel):
    target_proposal_id: StrictStr = Field(description="Proposal identifier under review.")
    target_step_number: StrictInt = Field(ge=1, description="Specific target trajectory step number.")
    flaw_type: Literal[
        "missing_evidence",
        "wrong_inference",
        "contradiction",
        ...
        "tool_misuse",
        "other",
    ] = Field(description="Review flaw type.")
    critique: StrictStr = Field(description="Concrete critique text.")
    evidence: List[DebateEvidenceToolInput] = Field(
        default_factory=list,
        description="Evidence supporting the critique.",
    )


class DebateReviewPayloadToolInput(ToolArgsModel):
    reviews: List[DebateReviewItemToolInput] = Field(
        default_factory=list,
        description="Strict JSON review payload.",
    )


class DebateReviewConcludeToolInput(ToolArgsModel):
    conclusion: DebateReviewPayloadToolInput = Field(description="Strict JSON review payload.")


class DebateRebuttalItemToolInput(ToolArgsModel):
    target_review_id: StrictStr = Field(description="Review identifier being answered.")
    response_mode: Literal["defend", "revise", "withdraw", "no_response"] = Field(
        description="How this rebuttal responds to the review item.",
    )
    response: StrictStr = Field(description="Rebuttal text.")
    evidence: List[DebateEvidenceToolInput] = Field(
        default_factory=list,
...
class DebateRebuttalPayloadToolInput(ToolArgsModel):
    rebuttals: List[DebateRebuttalItemToolInput] = Field(
        default_factory=list,
        description="Strict JSON rebuttal payload.",
    )
    revised_claim: Optional[StrictStr] = Field(
        default=None,
        description="Optional revised claim after addressing reviews.",
    )


class DebateRebuttalConcludeToolInput(ToolArgsModel):
    conclusion: DebateRebuttalPayloadToolInput = Field(description="Strict JSON rebuttal payload.")
...
_GENERIC_CONCLUDE_SCHEMA_BY_KIND: dict[str, Type[ToolArgsModel]] = {
    "proposal": DebateProposalConcludeToolInput,
    "reviews": DebateReviewConcludeToolInput,
    "rebuttals": DebateRebuttalConcludeToolInput,
}


def get_generic_conclude_args_schema(schema_kind: str) -> Type[ToolArgsModel]:
    return _GENERIC_CONCLUDE_SCHEMA_BY_KIND.get(str(schema_kind or "").strip(), GenericTextConcludeToolInput)
